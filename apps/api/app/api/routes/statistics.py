...
async def career(driver_key: str, db: AsyncSession = Depends(get_db)):
    try:
...
async def season(driver_key: str, season: int, db: AsyncSession = Depends(get_db)):
    try:
...
async def modern_race(
    driver_key: str,
    race_key: str,
    session_code: str = Query(default="race"),
