...
async def context_loaded_session_keys(session: AsyncSession, season: int) -> set[int]:
    result = await session.execute(text("""SELECT DISTINCT rs.provider_session_id FROM race_sessions rs JOIN races r ON r.id=rs.race_id WHERE r.season=:season AND rs.provider=:provider AND (EXISTS (SELECT 1 FROM session_race_control_events x WHERE x.session_id=rs.id) OR EXISTS (SELECT 1 FROM session_intervals x WHERE x.session_id=rs.id) OR EXISTS (SELECT 1 FROM session_pit_stops x WHERE x.session_id=rs.id) OR EXISTS (SELECT 1 FROM session_weather x WHERE x.session_id=rs.id))"""), {"season": season, "provider": PROVIDER})
...
async def upsert_race_control(session: AsyncSession, *, session_id: Any, session_key: int, rows: list[OpenF1RaceControlEvent], drivers: dict[int, Any | None]) -> int:
    ...
    for row in rows:
        params = {"session_id": session_id, "driver_entity_id": drivers.get(row.driver_number) if row.driver_number is not None else None, "provider": PROVIDER, "session_key": session_key, "driver_number": row.driver_number, "observed_at": row.observed_at, "category": row.category, "flag": row.flag, "scope": row.scope, "message": row.message, "lap_number": row.lap_number, "sector": row.sector, "source_url": f"https://api.openf1.org/v1/race_control?session_key={session_key}", "fetched_at": fetched_at, "raw": json.dumps(row.raw_payload)}
        await session.execute(text("""INSERT INTO session_race_control_events (session_id,driver_entity_id,provider,provider_session_key,provider_driver_number,observed_at,category,flag,scope,message,lap_number,sector,source_url,source_timestamp,fetched_at,raw_payload) VALUES (:session_id,:driver_entity_id,:provider,:session_key,:driver_number,:observed_at,:category,:flag,:scope,:message,:lap_number,:sector,:source_url,:observed_at,:fetched_at,CAST(:raw AS jsonb)) ON CONFLICT (provider,provider_session_key,observed_at,message) DO UPDATE SET driver_entity_id=EXCLUDED.driver_entity_id,category=EXCLUDED.category,flag=EXCLUDED.flag,scope=EXCLUDED.scope,lap_number=EXCLUDED.lap_number,sector=EXCLUDED.sector,fetched_at=EXCLUDED.fetched_at,raw_payload=EXCLUDED.raw_payload"""), params)
    return len(rows)
...
async def upsert_pit_stops(session: AsyncSession, *, session_id: Any, session_key: int, rows: list[OpenF1PitStop], drivers: dict[int, Any | None]) -> int:
    ...
    for row in rows:
        ...
        await session.execute(text("""INSERT INTO session_pit_stops (session_id,driver_entity_id,provider,provider_session_key,provider_driver_number,lap_number,observed_at,lane_duration_seconds,stop_duration_seconds,source_url,source_timestamp,fetched_at,raw_payload) VALUES (:session_id,:driver_entity_id,:provider,:session_key,:driver_number,:lap_number,:observed_at,:lane_duration,:stop_duration,:source_url,:observed_at,:fetched_at,CAST(:raw AS jsonb)) ON CONFLICT (provider,provider_session_key,provider_driver_number,lap_number,observed_at) DO UPDATE SET driver_entity_id=EXCLUDED.driver_entity_id,lane_duration_seconds=EXCLUDED.lane_duration_seconds,stop_duration_seconds=EXCLUDED.stop_duration_seconds,fetched_at=EXCLUDED.fetched_at,raw_payload=EXCLUDED.raw_payload"""), params)
    return len(rows)
