...
async def load_season_races(session: AsyncSession, season: int) -> list[dict[str, Any]]:
    result = await session.execute(
        text(
            ...
            FROM races
            WHERE season = :season AND start_at IS NOT NULL
            ORDER BY round
            """
        ),
        {"season": season},
...
def match_meetings_to_races(
    meetings: list[OpenF1Meeting], races: list[dict[str, Any]]
) -> dict[int, Any]:
...
async def upsert_meeting_links(
    session: AsyncSession,
    meetings: list[OpenF1Meeting],
    race_matches: dict[int, Any],
) -> int:
    written = 0
    for meeting in meetings:
        ...
        await session.execute(
            text(
                """
                INSERT INTO race_provider_ids (
                    race_id, provider, provider_id, provider_entity_type,
                    source_url, fetched_at, raw_metadata
                ) VALUES (
                    :race_id, :provider, :provider_id, 'meeting', :source_url,
                    now(), CAST(:raw_metadata AS jsonb)
                )
            ...
            ),
            {
                "race_id": race_id,
                "provider": PROVIDER,
                "provider_id": str(meeting.meeting_key),
            ...
            result = await session.execute(
                text(
                    ...
                    INSERT INTO race_sessions (
                        race_id, provider, provider_session_id, provider_meeting_id,
                        session_code, session_name, session_type, sequence, starts_at,
                        ends_at, gmt_offset, is_cancelled, source_url, fetched_at,
                        raw_metadata
                    ) VALUES (
                        :race_id, :provider, :provider_session_id, :provider_meeting_id,
                        :session_code, :session_name, :session_type, :sequence, :starts_at,
                ...
                {
                    "race_id": race_matches[meeting_key],
                    "provider": PROVIDER,
                    "provider_session_id": str(item.session_key),
                    "provider_meeting_id": str(item.meeting_key),
                    "session_code": item.session_code,
                    "session_name": item.session_name,
                    "session_type": item.session_type,
                    "sequence": sequence,
                    "starts_at": item.date_start,
                    "ends_at": item.date_end,
...
async def _resolve_driver_entity(
    session: AsyncSession, season: int, driver: OpenF1Driver
) -> tuple[Any | None, Any | None]:
    by_number = await session.execute(
        text(
            """
            SELECT p.entity_id AS driver_entity_id, t.entity_id AS team_entity_id
            FROM team_person_roles tpr
            JOIN persons p ON p.id = tpr.person_id
            JOIN teams t ON t.id = tpr.team_id
            WHERE tpr.season = :season
              AND tpr.role = 'driver'
              AND tpr.car_number = :car_number
            LIMIT 1
            """
        ),
        {"season": season, "car_number": driver.driver_number},
    )
    resolved = by_number.mappings().first()
    if resolved is not None:
        return resolved["driver_entity_id"], resolved["team_entity_id"]

    aliases = [value for value in (driver.full_name, driver.last_name, driver.name_acronym) if value]
        ...
        result = await session.execute(
            text(
                ...
                WHERE e.entity_type = 'person'
                  AND lower(ea.alias) = ANY(CAST(:aliases AS text[]))
                  AND ea.enabled = true
                  AND (ea.valid_from_season IS NULL OR ea.valid_from_season <= :season)
                  AND (ea.valid_to_season IS NULL OR ea.valid_to_season >= :season)
                ORDER BY ea.confidence DESC
                LIMIT 1
                """
            ),
            {"aliases": [item.lower() for item in aliases], "season": season},
        )
        ...
        result = await session.execute(
            text(
                ...
                WHERE e.entity_type = 'team'
                  AND lower(ea.alias) = :team_name
                  AND ea.enabled = true
                ORDER BY ea.confidence DESC
                LIMIT 1
                """
            ),
            {"team_name": driver.team_name.lower()},
        )
...
) -> tuple[dict[int, tuple[Any | None, Any | None]], int]:
        ...
        await session.execute(
            text(
                ...
                INSERT INTO session_entries (
                    session_id, provider, driver_provider_id, driver_number,
                    driver_entity_id, team_entity_id, broadcast_name, first_name,
                    last_name, full_name, name_acronym, team_name, team_colour,
                    headshot_reference_url, source_url, raw_metadata
                ) VALUES (
                    :session_id, :provider, :driver_provider_id, :driver_number,
                    :driver_entity_id, :team_entity_id, :broadcast_name, :first_name,
                    :last_name, :full_name, :name_acronym, :team_name, :team_colour,
                    :headshot_reference_url, :source_url, '{}'::jsonb
                )
                """
            ),
            {
                ...
                "provider": PROVIDER,
                "driver_provider_id": str(driver.driver_number),
                ...
                "headshot_reference_url": driver.headshot_url,
...
async def replace_session_results(
    session: AsyncSession,
    *,
    session_id: Any,
    session_key: int,
    rows: list[OpenF1SessionResult],
    entity_mappings: dict[int, tuple[Any | None, Any | None]],
) -> int:
        ...
        await session.execute(
            text(
                ...
                INSERT INTO session_results (
                    session_id, provider, provider_result_id, driver_provider_id,
                    driver_number, driver_entity_id, team_entity_id, position,
                    dnf, dns, dsq, number_of_laps, duration_seconds, q1_seconds,
                    q2_seconds, q3_seconds, gap_to_leader_seconds, gap_to_leader_text,
            ...
            ),
            {
                "session_id": session_id,
                "provider": PROVIDER,
                "provider_result_id": f"{session_key}:{row.driver_number}",
                "driver_provider_id": str(row.driver_number),
                ...
                "gap_to_leader_text": row.gap_to_leader_text,
