...
def _bounded_candidates(sessions: list[OpenF1Session], cached_keys: set[int], *, limit: int=2, now: datetime | None=None) -> list[OpenF1Session]:
    ...
    if latest.session_key not in {item.session_key for item in selected}: selected.append(latest)
...
async def sync_openf1(season: int, *, telemetry_limit: int=2) -> dict[str, object]:
    settings=get_settings(); headers={"User-Agent":settings.source_user_agent,"Accept":"application/json"}
    ...
    async with httpx.AsyncClient(timeout=timeout,headers=headers) as http_client:
        ...
        meetings=await client.meetings(season); provider_sessions=await client.sessions(season)
        ...
        candidates,reused_keys=_result_candidates(matched_sessions,cached_keys); results_written=entries_written=unresolved_entries=0; fetched_session_keys=[]
        for item in sorted(candidates,key=lambda row:row.date_start):
            drivers=await client.drivers(item.session_key); results=await client.session_results(item.session_key)
            async with SessionLocal() as db:
                async with db.begin():
            ...
            fetched_session_keys.append(item.session_key); entries_written+=len(drivers); results_written+=len(results); unresolved_entries+=unresolved
        telemetry_summary=[]
        for item in _telemetry_candidates(matched_sessions,telemetry_cached,limit=telemetry_limit):
            laps=await client.laps(item.session_key); stints=await client.stints(item.session_key); positions=await client.positions(item.session_key)
            async with SessionLocal() as db:
                async with db.begin():
                    driver_map=await driver_entity_map(db,session_ids[item.session_key]); lap_count=await upsert_laps(db,session_id=session_ids[item.session_key],session_key=item.session_key,rows=laps,drivers=driver_map); stint_count=await upsert_stints(db,session_id=session_ids[item.session_key],session_key=item.session_key,rows=stints,drivers=driver_map); position_count=await upsert_positions(db,session_id=session_ids[item.session_key],session_key=item.session_key,rows=positions,drivers=driver_map)
        ...
        for item in _bounded_candidates(matched_sessions,context_cached,limit=telemetry_limit):
            race_control=parse_race_control(await client._get("race_control",session_key=item.session_key)); intervals=parse_intervals(await client._get("intervals",session_key=item.session_key)); pits=parse_pit_stops(await client._get("pit",session_key=item.session_key)); weather=parse_weather(await client._get("weather",session_key=item.session_key))
            async with SessionLocal() as db:
                async with db.begin():
