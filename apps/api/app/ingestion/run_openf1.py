...
async def _context_payload(client: OpenF1Client, endpoint: str, session_key: int):
    return await client._get(endpoint, session_key=session_key)
...
async def sync_openf1(season: int, *, telemetry_limit: int = 2) -> dict[str, object]:
    ...
    async with httpx.AsyncClient(timeout=timeout, headers=headers) as http_client:
        ...
        for item in _bounded_candidates(matched_sessions, context_cached, limit=telemetry_limit):
            race_control = parse_race_control(await _context_payload(client, "race_control", item.session_key))
            intervals = parse_intervals(await _context_payload(client, "intervals", item.session_key))
            pits = parse_pit_stops(await _context_payload(client, "pit", item.session_key))
            weather = parse_weather(await _context_payload(client, "weather", item.session_key))
            async with SessionLocal() as db:
                async with db.begin():
                    driver_map = await driver_entity_map(db, session_ids[item.session_key])
...
def main() -> None:
    ...
    parser.add_argument("--telemetry-limit", type=int, default=2, help="Maximum completed sessions to fetch telemetry and race context for per run")
