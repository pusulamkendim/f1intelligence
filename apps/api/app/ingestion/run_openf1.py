...
async def _audit(session, *, dataset: str, season: int, records_seen: int, records_written: int, metadata: dict[str, object] | None = None) -> None:
    await session.execute(text("""
        INSERT INTO data_sync_runs (provider, dataset, season, status, source_url, completed_at, records_seen, records_written, metadata)
        VALUES ('openf1', :dataset, :season, 'succeeded', :source_url, now(), :records_seen, :records_written, CAST(:metadata AS jsonb))
    """), {"dataset": dataset, "season": season, "source_url": f"{OPENF1_BASE_URL}/{dataset}", "records_seen": records_seen, "records_written": records_written, "metadata": json.dumps(metadata or {})})
...
def _telemetry_candidates(sessions: list[OpenF1Session], cached_keys: set[int], *, limit: int = 2, now: datetime | None = None) -> list[OpenF1Session]:
    """Fetch only a small historical backlog; refresh the latest completed session."""
...
async def sync_openf1(season: int, *, telemetry_limit: int = 2) -> dict[str, object]:
    ...
    async with httpx.AsyncClient(timeout=timeout, headers=headers) as http_client:
        ...
        telemetry_sessions = _telemetry_candidates(matched_sessions, telemetry_cached, limit=telemetry_limit)
        telemetry_summary: list[dict[str, int]] = []
        for item in telemetry_sessions:
...
def main() -> None:
    ...
    parser.add_argument("--telemetry-limit", type=int, default=2, help="Maximum completed sessions to fetch telemetry for per run")
