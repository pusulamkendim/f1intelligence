            ...
            audit = await session.execute(
                text(
                    ...
                    SELECT
                        ...
                        COUNT(*) FILTER (WHERE precision = 'season')::int AS season,
                        COUNT(*) FILTER (WHERE precision = 'race')::int AS race,
                        COUNT(*) FILTER (WHERE precision = 'session')::int AS session,
                        COUNT(*) FILTER (WHERE precision = 'stint')::int AS stint,
                        COUNT(*) FILTER (WHERE precision = 'lap')::int AS lap,
                        COUNT(*) FILTER (WHERE precision = 'timestamp')::int AS timestamp,
                        COUNT(*) FILTER (WHERE temporal_relation = 'scheduled_for')::int
                            AS scheduled_for,
                        COUNT(*) FILTER (WHERE temporal_relation = 'effective_from')::int
                            AS effective_from
